...
export default function Features({ ar = false }) {
  const style = "grid gap-8 sm:grid-cols-2 md:gap-12 xl:grid-cols-3 xl:gap-16"
  const ttitle = ar ? "خدماتنا" : " Our Services  ";
  const tsubtitle = ar ? "     نحن نقدم خدمتنا على مدار الساعة طوال أيام الأسبوع في جميع أنحاء الرياض  " : "       We provide our 24/7 service accross riyadh   ";
  const card = ar
    ?  [
    ...
    : [
      ...
      { title: "Special Ocasions", para: "Elevate your special occasions with Sweet Water Supply, providing impeccable water service that adds a touch of elegance and sophistication to your memorable events", bg: "special.webp" },
        ...
        <div className="mx-auto max-w-screen-2xl px-4 md:px-8">
          ...
          <div className={style}>
            {card.map((item, index) => { return(
              <FeautreP key={index} title={item.title} para={item.para} bg={item.bg} />)
            })
